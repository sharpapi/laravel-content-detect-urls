...
All notable changes to `laravel-content-detect-urls` will be documented in this file.

## v1.1.0 - 2026-09-24

- Added a Laravel Boost skill (`resources/boost/skills/sharpapi-content-detect-urls/SKILL.md`) so AI agents can discover the async submit/`fetchResults()` flow, the queued-job recipe, result shape and testing approach.
...
- Docs: fixed the README: the usage example claimed `detectUrls()` returns the URLs; it returns a status URL for `fetchResults()`.
